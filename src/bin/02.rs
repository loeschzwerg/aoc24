...
fn eliminate_invalid(u: u8) -> u8 {
    if u.count_zeros() < 2 { u }
    else if u.count_zeros() > 2 { 0 }
    else if u.leading_ones() + u.trailing_ones() == 6 { u } // eliminate intermittent 1
    else { 0 }
}

fn asc(a:u64, b:u64) -> bool {a < b} // identifies ascending in negation
fn desc(a:u64, b:u64) -> bool {a > b} // identifies descending in negation
fn diff(a:u64,b:u64) -> bool {let d = (a as i64).abs_diff(b as i64); d > 0 && d < 4}
fn asc_check(a:u64,b:u64) -> bool{ asc(a,b) && diff(a,b) }
fn desc_check(a:u64,b:u64) -> bool{ desc(a,b) && diff(a,b) }
pub fn part_two(input: &str) -> Option<u32> {
    let setup = input.lines()
    ...
    let cleaned: Vec<_>= cleanup
        ...
        .filter(|(u, a, d)| {
            ...
            if *d == 0 {
                ...
                single_bit_cmp(*u, M[i], B[i], M[min(i+2, M.len()-1)], B[min(i+2, B.len()-1)], asc_check)
            } else if *a == 0 {
                single_bit_cmp(*u, M[i-1], B[i-1], M[i+1], B[i+1], desc_check) ||
                single_bit_cmp(*u, M[i], B[i], M[min(i+2, M.len()-1)], B[min(i+2,B.len()-1)], desc_check)
            } else { dbg!(i, a, d, u); false }
        })
        .collect();
