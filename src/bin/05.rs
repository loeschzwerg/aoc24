...
pub fn infer(
    _cache: &std::collections::HashSet<(u32, u32)>,
    facts: &[(u32, u32)],
) -> Option<Inference<(u32, u32)>> {
...
pub fn part_one(input: &str) -> Option<u32> {
    let (prelim, seqs): (Vec<&str>, Vec<&str>) = input.lines().partition(|x| x.contains('|'));
    let facts: Vec<(u32, u32)> = prelim
        .iter()
    ...
    let binding = k.difference(&v).cloned().collect::<Vec<_>>();
    let begin: &u32 = binding.first().unwrap_or(k.first().unwrap());
    ...
    let indexed: BTreeMap<u32, usize> =
        ordered
            .iter()
            .enumerate()
            .fold(BTreeMap::new(), |mut acc, (i, v)| {
                acc.insert(*v, i);
                acc
            });
    ...
    let result = seqs
        .into_iter()
        .map(|x| x.split(',').filter_map(|x| x.parse::<u32>().ok()).collect())
